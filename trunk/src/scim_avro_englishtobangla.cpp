...
WideString cEnglishToBangla::Convert(const String EnglishT) {
  ...
  */
  ...
  // FIXME:  Eliminate the following two lines and uncomment above when
...
void cEnglishToBangla::InitVars() {
  //Bangla Numbers
  b_0 = ucs4_t(0x9E6);
  b_1 = ucs4_t(0x9E7);
  b_2 = ucs4_t(0x9E8);
...
void cEnglishToBangla::CutText(String inputEStr,
                               String &outSIgnore,
                               String &outMidMain,
                               String &outEIgnore) {
  int i = 0;
  ...
  for (i = 1; i <= EStrLen; i++) {
    ...
    if ((tStr == "~") || (tStr == "!") || (tStr == "@") || (tStr == "#") ||
      ...
      if (i - 1 >= 1) {
        if (inputEStr.substr(i - 2, 1) == "." ||
            inputEStr.substr(i - 2, 1) == ":") {
          p = i;
        } else {
          goto ExitLabel1;
        }
...
WideString cEnglishToBangla::MyConvert() {
  ...
  do {
    ...
    //-----------------START Consonent Generation---------------
      ...
      if ((Consonent(PrevT()) == false) && (Begining() != true)) {
        ...
        if (PrevT() == "r") {
          if ((Consonent(PrevTEx(2))) && (PrevTEx(2) != "r") &&
              (PrevTEx(2) != "y") && (PrevTEx(2) != "w") &&
              (PrevTEx(2) != "x")) {
            //Previous character is R-Fola, don't add ZWJ
            AddRs (b_Hasanta + b_Z);
          } else {
...
String cEnglishToBangla::CorrectCase (String inputT) {
  ...
  for (i = 1; i <= l; i++) {
    ...
    if ((T == "a") || (T == "A") || (T == "o") || (T == "O") || (T == "i") ||
        (T == "I") || (T == "u") || (T == "U")) {
      s = s + T;
    } else if ((T == "d") || (T == "D") || (T == "g") || (T == "G") ||
               (T == "j") || (T == "J") || (T == "n") || (T == "N") ||
               (T == "r") || (T == "R") || (T == "s") || (T == "S") ||
               (T == "t") || (T == "T") || (T == "y") || (T == "Y") ||
               (T == "z") || (T == "Z")) {
...
void cEnglishToBangla::R() {
  ...
  if ((Consonent(PrevT()) == true) && (PrevT() != "r") && (PrevT() != "y") &&
      (PrevT() != "w") && (PrevT() != "x") && (PrevT() != "Z")) {
...
bool cEnglishToBangla::Cnv(String Compare, WideString IfTrue) {
  int i = Compare.length();
  String tmp = pEnglishText.substr(pos - 1, i);
...
void cEnglishToBangla::AddRs(WideString T) {
  Rs = Rs + T;
  pos = pos + 1;
}

void cEnglishToBangla::AddRsEx(WideString T, int p) {
  Rs = Rs + T;
  pos = pos + p;
}

String cEnglishToBangla::PrevT() {
  int i = 0;
...
String cEnglishToBangla::PrevTEx (int Position) {
  int i = 0;
...
String cEnglishToBangla::NextT() {
  return pEnglishText.substr(pos, 1);
}

String cEnglishToBangla::NextTEx (int Length, int skipstart) {
  if (Length < 1) {
    Length = 1;
  }
...
bool cEnglishToBangla::Vowel(String T) {
  std::transform (T.begin(), T.end(), T.begin(), tolower);
...
bool cEnglishToBangla::Consonent(String T) {
  std::transform (T.begin(), T.end(), T.begin(), tolower);

  if ((T == "b") || (T == "c") || (T == "d") || (T == "f") || (T == "g") ||
      (T == "h") || (T == "j") || (T == "k") || (T == "l") || (T == "m") ||
      (T == "n") || (T == "p") || (T == "q") || (T == "r") || (T == "s") ||
      (T == "t") || (T == "v") || (T == "w") || (T == "x") || (T == "y") ||
      (T == "z")) {
...
bool cEnglishToBangla::Begining() {
